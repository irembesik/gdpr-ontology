    ...
    var rgraph = new $jit.RGraph({
        ...
        onBeforeCompute: function(node){
            ...
            var tpl = function(data) {
              return '<h4><a href="' + data.link + '" target="_blank">' + data.location + ' </a></h4>' + data.description;
            }
            var html = '';
              console.log(node.data, Array.isArray(node.data))

            if (node.data.multiple) {
              console.log(node.data)
              node.data.multiple.forEach(function(data) {
                console.log(data)
                html +=  tpl(data) + '<hr>';
              })
            } else {
